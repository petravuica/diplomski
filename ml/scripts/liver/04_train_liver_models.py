...
# Precision za pozitivnu klasu 1.
# zero_division=0 uklanja upozorenja kod Dummy modela.
SCORING = {
    "accuracy": "accuracy",
...
def load_clean_dataset() -> pd.DataFrame:
    """
    Učitava očišćeni jetreni dataset i provjerava strukturu.

    Nedostajuće vrijednosti dopuštene su samo u A/G omjeru
    jer će ih Pipeline naknadno imputirati medijanom.
    """
    if not CLEAN_DATASET_PATH.exists():
        raise FileNotFoundError(
            "Očišćeni jetreni dataset nije pronađen:\n"
            f"{CLEAN_DATASET_PATH}\n\n"
            "Prvo pokreni 03_liver_clean_dataset.py."
        )
    ...
    if (
        df[required_without_ag_ratio]
        .isna()
        .any()
        .any()
    ):
        raise ValueError(
            "Dataset sadrži nedostajuće vrijednosti "
...
def save_train_test_split(
    ...
    """
    Sprema korištene train/test skupove radi reproducibilnosti.
    """
...
def create_preprocessor() -> ColumnTransformer:
    ...
    Numeričke značajke:
    - missing vrijednosti imputiraju se medijanom
    - zatim se standardiziraju

    Kategorijske značajke:
    - missing vrijednosti imputiraju se najčešćom vrijednošću
    - spol se pretvara one-hot kodiranjem
    """
...
def create_model_configurations() -> dict:
    """
    Definira modele i hiperparametre za GridSearchCV.
    """
...
def train_models(
    ...
    """
    Trenira i optimizira modele samo na trening skupu.
    """
    ...
    for model_name, configuration in (
        ...
        print("-" * 80)
        print(
            f"Treniranje modela: {model_name}"
        )
...
def select_and_save_best_model(
    ...
    """
    Odabire model s najboljim CV balanced accuracy rezultatom.
    """
...
def save_training_outputs(
    ...
    """
    Sprema detaljne CV rezultate i sažetak usporedbe.
    """
...
def save_training_report(
    ...
    """
    Sprema tekstualni izvještaj o treningu.
    """
    full_distribution = (
    ...
    with TRAINING_REPORT_PATH.open(
        ...
        report.write(
            "Pozitivna klasa 1: jetreni pacijent\n"
        )
        report.write(
            "Negativna klasa 0: osoba bez oznake "
            "jetrene bolesti\n\n"
        )
        ...
        report.write(
            "3. IMPUTACIJA NEDOSTAJUĆIH VRIJEDNOSTI\n"
        )
        report.write("-" * 85 + "\n")
        report.write(
            "Numeričke missing vrijednosti imputiraju se "
            "medijanom unutar Pipelinea.\n"
        )
        report.write(
            "Missing A/G omjeri u trening skupu: "
            f"{missing_ag_train}\n"
        )
        report.write(
            "Missing A/G omjeri u testnom skupu: "
            f"{missing_ag_test}\n\n"
        )

        report.write(
            "4. RASPODJELA KLASA U CIJELOM DATASETU\n"
        )
        report.write("-" * 85 + "\n")
        report.write(
        ...
        report.write(
            "5. RASPODJELA KLASA U TRENING SKUPU\n"
        )
        report.write("-" * 85 + "\n")
        report.write(
        ...
        report.write(
            "6. RASPODJELA KLASA U TESTNOM SKUPU\n"
        )
        report.write("-" * 85 + "\n")
        report.write(
        ...
        report.write(
            "7. USPOREDBA MODELA NA TRENING SKUPU\n"
        )
        report.write("-" * 85 + "\n")
        report.write(
            comparison_df[
        ...
        report.write(
            "9. METODOLOŠKA NAPOMENA\n"
        )
        report.write("-" * 85 + "\n")
        report.write(
            "Dataset je stratificirano podijeljen na trening "
            "i testni skup. Imputacija, standardizacija i "
            "kodiranje kategorijskih vrijednosti nalaze se "
            "unutar Pipelinea, čime se sprječava curenje "
            "podataka. Optimizacija hiperparametara provedena "
            "je isključivo na trening skupu peterostrukom "
            "stratificiranom unakrsnom validacijom. Testni "
            "skup nije korišten tijekom odabira modela.\n"
        )
...
def save_training_metadata(
    ...
    """
    Sprema ključne metapodatke treninga.
    """
    metadata = {
        "created_at_utc": (
...
def print_summary(
    ...
    """
    Ispisuje sažetak treninga.
    """
    print("\n" + "=" * 80)
    print("SAŽETAK TRENIRANJA JETRENIH MODELA")
    print("=" * 80)
    ...
    print(
        "\nMissing A/G vrijednosti u trening skupu:"
    )
    print(
        X_train[
            "Albumin_and_Globulin_Ratio"
    ...
    print(
        "\nMissing A/G vrijednosti u testnom skupu:"
    )
    print(
        X_test[
            "Albumin_and_Globulin_Ratio"
...
def main() -> None:
    """
    Glavna funkcija trening skripte.
    """
    create_output_directories()

    print(
        "Učitavanje očišćenog jetrenog dataseta..."
    )

    df = load_clean_dataset()

    print(
        "Izrada stratificiranog train/test splita..."
    )
    ...
    print(
        "Treniranje i optimizacija modela..."
    )
    ...
    print(
        "Spremanje izvještaja o treningu..."
    )
